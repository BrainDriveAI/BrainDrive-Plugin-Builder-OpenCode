#!/usr/bin/env python3
"""
PluginTemplate Plugin Lifecycle Manager

Handles install/update/delete operations for the PluginTemplate plugin
...
try:
    ...
    logger.info("PluginTemplate: Using BaseLifecycleManager from app.plugins")
except ImportError:
    try:
        import sys
        ...
        if os.path.exists(backend_path):
            ...
            logger.info(f"PluginTemplate: Using BaseLifecycleManager from: {backend_path}")
        else:
            # Minimal implementation for remote installations
            logger.warning(f"PluginTemplate: BaseLifecycleManager not found, using minimal implementation")
            ...
            logger.info("PluginTemplate: Using minimal BaseLifecycleManager implementation")
            
    except ImportError as e:
        logger.error(f"PluginTemplate: Failed to import BaseLifecycleManager: {e}")
        raise ImportError("PluginTemplate plugin requires BaseLifecycleManager")


class PluginTemplateLifecycleManager(BaseLifecycleManager):
    """Lifecycle manager for PluginTemplate plugin"""
    ...
    def __init__(self, plugins_base_dir: str = None):
        ...
        self.plugin_data = {
            ...
            "category": "utility",
            "official": False,
            "author": "BrainDrive Plugin Builder",
            "compatibility": "1.0.0",
            "scope": "PluginTemplate",
            "bundle_method": "webpack",
            "bundle_location": "dist/remoteEntry.js",
            "is_local": False,
            "long_description": "A BrainDrive plugin generated by the Plugin Builder",
        ...
        self.module_data = [
            {
                "name": "PluginTemplateModule",
                "display_name": "Plugin Template Module",
        ...
        try:
            ...
            for item in source_dir.rglob('*'):
                ...
                try:
                    ...
                    logger.warning(f"PluginTemplate: Failed to copy {relative_path}: {e}")
                    continue
            ...
            logger.info(f"PluginTemplate: Copied {len(copied_files)} files to {target_dir}")
    ...
    async def _validate_installation_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                for field in required_fields:
                    if field not in package_data:
                        return {
                            'valid': False,
                            'error': f'PluginTemplate: package.json missing field: {field}'
                        }
                        
            except (json.JSONDecodeError, FileNotFoundError) as e:
                return {
                    'valid': False,
                    'error': f'PluginTemplate: Invalid package.json: {e}'
                }
            ...
            if bundle_path.stat().st_size == 0:
                return {
                    'valid': False,
                    'error': 'PluginTemplate: Bundle file is empty'
                }
            
            logger.info(f"PluginTemplate: Validation passed for user {user_id}")
            return {'valid': True}
    ...
    async def _get_plugin_health_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"PluginTemplate: Error checking health: {e}")
            return {
                'healthy': False,
                'details': {'error': str(e)}
            }
    ...
    async def _create_database_records(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"PluginTemplate: Creating database records for {plugin_id}")
            ...
            if verify_row:
                logger.info(f"PluginTemplate: Created records for {plugin_id}")
            else:
            ...
            logger.error(f"PluginTemplate: Error creating database records: {e}")
            await db.rollback()
            return {'success': False, 'error': str(e)}
    ...
    async def _delete_database_records(self, user_id: str, plugin_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"PluginTemplate: Deleted records for {plugin_id}")
            return {'success': True, 'deleted_modules': deleted_modules}
            
        except Exception as e:
            logger.error(f"PluginTemplate: Error deleting records: {e}")
            await db.rollback()
            return {'success': False, 'error': str(e)}
    ...
    async def _create_plugin_page(self, user_id: str, db: AsyncSession, modules_created: List[str]) -> Dict[str, Any]:
        ...
        try:
            # Use plugin slug as route
            plugin_route = self.plugin_data['plugin_slug']
            ...
            if existing:
                ...
                logger.info(f"PluginTemplate: Page already exists for {user_id}", page_id=existing_page_id)
            ...
            if not module_id:
                logger.error(f"PluginTemplate: Failed to resolve module ID for {user_id}")
            ...
            await db.commit()
            logger.info(f"PluginTemplate: Created page for {user_id}", page_id=page_id)
            return {"success": True, "page_id": page_id, "created": True}

        except Exception as e:
            await db.rollback()
            logger.error(f"PluginTemplate: Failed to create page for {user_id}: {e}")
            return {"success": False, "error": str(e)}
    ...
    async def _delete_plugin_page(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            await db.commit()
            logger.info(f"PluginTemplate: Deleted page for {user_id}", deleted_rows=result.rowcount)
            return {"success": True, "deleted_rows": result.rowcount}

        except Exception as e:
            await db.rollback()
            logger.error(f"PluginTemplate: Failed to delete page for {user_id}: {e}")
            return {"success": False, "error": str(e)}
    ...
    async def install_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"PluginTemplate: Starting installation for {user_id}")
            
            existing_check = await self._check_existing_plugin(user_id, db)
            if existing_check['exists']:
                logger.warning(f"PluginTemplate: Already installed for {user_id}")
                return {
                    'success': False,
    ...
    async def get_plugin_status(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"PluginTemplate: Error checking status: {e}")
